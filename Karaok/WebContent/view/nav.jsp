		...
		$(document).on("click","#logout",function(){
			...
			$.ajax({
				...
				success:function(response){
					alert(response.msg);
					$('#QnA').hide();
					$('#QnAMother').prepend('<li id="QnA"><a href="main.ok" onclick="alert("로그인이 필요합니다.")">1:1문의</a></li>');
					...
					if($(location).attr('href')=='http://localhost/Karaok/main.ok'){ */
					$('#qnaSmall').remove();
					$('#qnaPreview').append('<small id="qnaSmall"><small><a href="main.ok" id="qna" onclick="alert("로그인이 필요합니다.")">더보기</a></small></small></h2>');
					/* } */
				},
				dataType : "json",
	            error: function(error,status,xhr) {
        ...
        <li class="dropdown">
          ...
          <ul class="dropdown-menu" role="menu" id="QnAMother">
          	<%if(request.getSession().getAttribute("currentId") == null) {%>
            	<li id="QnA"><a href="main.ok" onclick="alert('로그인이 필요합니다.')">1:1문의</a></li>
            <%} else { %>
